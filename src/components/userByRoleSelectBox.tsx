...
import { GET_USER_BY_ROLES, GET_WORKERS } from "../api/myQueries";
      ...
      return (
        <div className={`input_container gap_12 ${isEmpty ? 'error' : ''}`}>
          ...
          <select className="editable_input width_100" id={name} name={name} onChange={onChange} value={value} disabled={disabled}>
              ...
              {worker.getUserByRole.map((option:PricaWorker)=><option key={option._id} value={option._id}>{option.name}</option>)}
          </select>
      </div>
      )
    }
