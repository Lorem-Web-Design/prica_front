...
export default function RqCard({ cardInfo }: RQCard) {
  ...
  return (
    <>
      ...
      />
      <div
        className={`rqCardContainer ${isVisible ? "" : "hide"} ${
          cardInfo.hide ? "hide" : ""
        }`}
        onClick={() => navigate(`/requisicion/viewer/${cardInfo._id}`)}
        ref={cardReference}
      >
            ...
            <li
              onClick={() => {
                const deleteConfirmed = confirm(
                  "¿Estás seguro que deseas eliminar esta RQ?"
                );
                if (deleteConfirmed) {
        ...
        {cardInfo.isEnded ? (
          <span className="green">FINALIZADA</span>
        ) : cardInfo.haveOC ? (
          <span className="blue">EN PROCESO DE COMPRA</span>
        ) : (
          <p className={`${cardInfo.isApproved ? "green" : "yellow"}`}>{`${
            cardInfo.isApproved ? "APROBADO" : "PENDIENTE"
          }`}</p>
        )}
      </div>
    </>
