          ...
          <div className="chipsContainer">
            {epp.classification.map((classification, index) => (
              <div className="primary_theme" key={index}>
                {classification.name}: {classification.amount} <span className="material-symbols-outlined delete_category" onClick={()=>deleteClassification(classification.id)}>x_circle</span>
              </div>
            ))}
          </div>
        </div>
