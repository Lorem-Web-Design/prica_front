import ELEMENT_CATEGORIES from "../settings/categories.json";
    ...
    return (
      <div className={`input_container gap_12 ${isEmpty ? 'error' : ''}`}>
        ...
        <select className="editable_input width_100" id={name} name={name} onChange={onChange} value={value} disabled={disabled}>
          {ELEMENT_CATEGORIES.categories.map(category=><option value={category.slug} key={category.slug}>{category.name}</option>)}
        </select>
    </div>
    )
  }
