...
type SelectBox = {
  ...
  isEmpty: boolean;
  value: string;
  handleFolderSelect:  (evt: SelectType) => void
};
...
export default function ParentChildrenSelectorForRemision({ label, name, isEmpty, value, handleFolderSelect }: SelectBox) {
  ...
  if (data) {
    ...
    return (
      ...
      <ChildrenByParentSelect parentId={parentId} handleFolderSelect={handleFolderSelect}/>
      </>
    );
  }
...
function ChildrenByParentSelect({parentId, handleFolderSelect}: ChildrenByParent){
    ...
    if (error) {
      return (
        <div className={`input_container gap_12`}>
          ...
          {/* @ts-ignore */}
          <Select styles={selectStyles}
            options={[{value: "UNDEFINED", label: "Seleccione un centro de costos"}]}
            onChange={handleFolderSelect}
            className="editable_input"
            placeholder="Selecciona una centro de costos..."
          />
    ...
    if (loading) {
      return (
        <div className={`input_container gap_12`}>
          ...
          {/* @ts-ignore */}
          <Select styles={selectStyles}
            options={[{value: "UNDEFINED", label: "Cargando información"}]}
            onChange={handleFolderSelect}
            className="editable_input"
            placeholder="Selecciona una centro de costos..."
          />
